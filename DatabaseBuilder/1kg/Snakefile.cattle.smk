...
for s in pep.samples:
    if s.cluster not in clusters:
        clusters[s.cluster] = {
            ...
            "rowdiff": [],
            "rowdiffCount": [],
            "partition": paritions[count%len(paritions)]
            }
        count+=1
...
for c in clusters.keys():
    ...
    for i in range(0,numSamples,batchSize):
        ...
        clusters[c]["batches"][batchID]= clusters[c]["unitigs"][start:end]
...
#        partition = lambda wildcards , attempt: getLowPartition(f"{wildcards.sample}", attempt),
    ...
    shell:
       	"""
	  /home/mshokrof/TheGreatGenotyper/build5/metagraph smoothCounts \
          -k {kSize}  \
...
rule metagraph_buildSmoothMainGraph_1:
    ...
    resources:
        ...
        partition = "bmm",
#        partition = lambda wildcards , attempt: clusters[f"{wildcards.cluster}"]['partition'],
	...
	#    mamba create -p ./metagraph.$$ -c bioconda -c conda-forge metagraph
        ...
        mkdir  -p {tmpFolder}$$/
        /home/mshokrof/TheGreatGenotyper/build5/metagraph build  \
        -k {kSize} \
        --mode canonical  \
        --mem-cap-gb {resources.mem_gb} \
        --disk-swap {tmpFolder}$$/  \
...
#        partition = lambda wildcards , attempt: clusters[f"{wildcards.cluster}"]['partition'],
    ...
    shell:
        ...
        /home/mshokrof/TheGreatGenotyper/build5/metagraph transform  \
            --to-fasta --primary-kmers \
	        -p {threads} \
            -o  {params.outputPrefix} \
            {input.graph} &>> {log}
...
#        "../../environment.yml"
    ...
    resources:
        ...
        mem_gb=70,
	    nodes = 1,
        runtime = lambda wildcards, attempt: 60 * 5 * attempt,
        partition = "bmm",
#        partition = lambda wildcards , attempt: clusters[f"{wildcards.cluster}"]['partition'],
        tmp= lambda wildcards: "%smaingraph_smooth%s/"%(tmpFolder,f"{wildcards.cluster}")
    log: outputFolder+"{cluster}/buildMainGraphlog"
...
#         "../../environment.yml"
	...
	/home/mshokrof/TheGreatGenotyper/build5/metagraph annotate  \
                -i {input.graph} \
                --anno-filename \
                --separately \
                --count-kmers --count-width 16 \
...
#        partition = lambda wildcards , attempt: clusters[f"{wildcards.cluster}"]['partition'],
	...
	/home/mshokrof/TheGreatGenotyper/build5/metagraph transform_anno  \
...
#        "env.yaml"
    ...
    shell:
        ...
        /home/mshokrof/TheGreatGenotyper/build5/metagraph relax_brwt  \
            -p {threads} \
            --relax-arity 32 \
