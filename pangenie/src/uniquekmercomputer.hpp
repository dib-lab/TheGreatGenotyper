...
public:
	...
	**/
	UniqueKmerComputer (KmerCounter* genomic_kmers, KmerCounter* read_kmers, VariantReader* variants, std::string chromosome, std::map<std::string, std::size_t> kmer_coverage);
	/** generates UniqueKmers object for each position, ownership of vector is transferred to the caller. **/
	void compute_unique_kmers(std::map<std::string,std::vector<UniqueKmers*> >* result, std::map<std::string, ProbabilityTable>* probabilities);
...
private:
	...
	std::map<std::string, std::size_t> kmer_coverage;
	...
	**/
	unsigned short compute_local_coverage(std::string sampleName,std::string chromosome, size_t var_index, size_t length);
};
