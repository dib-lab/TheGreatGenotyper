...
void JellyfishCounter::computeCorrectionStats(string filename) {
	...
	for (auto it = jf_ary->begin(); it != end; ++it) {
		...
		if ( (scaling_factor < -2) || (scaling_factor > 2) ) continue;
		size_t hist_ixd = (size_t) floor((scaling_factor + 2.0) / 0.01);
